...
public class EntityUtils {
	...
	public static void clearTarget(Entity target) {
		ImmutableArray<Entity> entities = engine.getEntitiesFor(Family.all(
		...
		for (Entity entity : entities) {
			...
			if (targetComp.getTarget() == target) {
				...
				// Dispatch a message to the entites FSM that there target was
				// removed from the engine
				if (Components.FSM.has(entity)) {
					FSMComponent fsm = Components.FSM.get(entity);
					MessageManager.getInstance().dispatchMessage(null, fsm,
							TelegramMessage.TARGET_REMOVED.ordinal(), target);
				}
			}
