...
public class WaveSystem extends EntitySystem {
	...
	public void initalizePoints(int points) {
		spawnPoints.clear();
		for (int i = 0; i < points; i++) {
			spawnPoints.add(Vector2Utils.random(256, 256));
		}
	}
	...
	public static void initalizeSpawns() {
		Task task = new Task() {
			...
			@Override
			public void run() {
				WaveSystem.spawnSquad(spawnPoints.random(),
						Constants.enemyFaction, 3);
			}
		};
	...
	public static void spawnSquad(Vector2 position, Faction faction, int members) {
		...
		if (faction != Constants.playerFaction) {
			Components.SQUAD.get(squad).setTarget(new Vector2(128, 128)); // TODO
																			// make
																			// dynamic
		}
	}
	...
	private void initalizeNextWave() {
		...
		wave.squadCount = MathUtils.random(1, 3);
